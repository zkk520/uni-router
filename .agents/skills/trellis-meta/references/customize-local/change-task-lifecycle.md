...
hooks:
  after_create:
    - "uv run python .trellis/scripts/hooks/my_sync.py create"
  after_start:
    - "uv run python .trellis/scripts/hooks/my_sync.py start"
  after_finish:
    - "uv run python .trellis/scripts/hooks/my_sync.py finish"
  after_archive:
    - "uv run python .trellis/scripts/hooks/my_sync.py archive"
```
...
1. Confirm the current task with `uv run python ./.trellis/scripts/task.py current --source`.
