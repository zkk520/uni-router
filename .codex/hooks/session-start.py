...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    if task_status == "completed":
        return (
            f"Status: COMPLETED\nTask: {task_title}\n"
            f"Next: Archive with `uv run python ./.trellis/scripts/task.py archive {task_dir.name}` "
            "or start a new task."
        )
...
def _build_compact_current_state(
    ...
    if get_tasks_dir and iter_active_tasks:
        try:
            ...
            lines.append(
                f"Active tasks: {task_count} total. Use `uv run python ./.trellis/scripts/task.py list --mine` only if needed."
            )
        except Exception:
            pass
...
def _build_workflow_toc(workflow_path: Path) -> str:
    ...
    out_lines = [
        ...
        "Full guide: .trellis/workflow.md. Step detail: `uv run python ./.trellis/scripts/get_context.py --mode phase --step <X.Y>`.",
        "",
    ]
