...
```bash
uv run python ./.trellis/scripts/init_developer.py <your-name>
```
...
```bash
uv run python ./.trellis/scripts/get_context.py --mode packages   # list packages / layers
```
...
```bash
# Task lifecycle
uv run python ./.trellis/scripts/task.py create "<title>" [--slug <name>] [--parent <dir>]
uv run python ./.trellis/scripts/task.py start <name>          # set active task (session-scoped when available)
uv run python ./.trellis/scripts/task.py current --source      # show active task and source
uv run python ./.trellis/scripts/task.py finish                # clear active task (triggers after_finish hooks)
uv run python ./.trellis/scripts/task.py archive <name>        # move to archive/{year-month}/
uv run python ./.trellis/scripts/task.py list [--mine] [--status <s>]
uv run python ./.trellis/scripts/task.py list-archive

# Code-spec context (injected into implement/check agents via JSONL).
# `implement.jsonl` / `check.jsonl` are seeded on `task create` for sub-agent-capable
# platforms; the AI curates real spec + research entries during planning when needed.
uv run python ./.trellis/scripts/task.py add-context <name> <action> <file> <reason>
uv run python ./.trellis/scripts/task.py list-context <name> [action]
uv run python ./.trellis/scripts/task.py validate <name>

# Task metadata
uv run python ./.trellis/scripts/task.py set-branch <name> <branch>
uv run python ./.trellis/scripts/task.py set-base-branch <name> <branch>    # PR target
uv run python ./.trellis/scripts/task.py set-scope <name> <scope>

# Hierarchy (parent/child)
uv run python ./.trellis/scripts/task.py add-subtask <parent> <child>
uv run python ./.trellis/scripts/task.py remove-subtask <parent> <child>

# PR creation
uv run python ./.trellis/scripts/task.py create-pr [name] [--dry-run]
```

> Run `uv run python ./.trellis/scripts/task.py --help` to see the authoritative, up-to-date list.
...
```bash
uv run python ./.trellis/scripts/add_session.py --title "Title" --commit "hash" --summary "Summary"
```

### Context Script

```bash
uv run python ./.trellis/scripts/get_context.py                            # full session runtime
uv run python ./.trellis/scripts/get_context.py --mode packages            # available packages + spec layers
uv run python ./.trellis/scripts/get_context.py --mode phase --step <X.Y>  # detailed guide for a workflow step
```
...
```bash
uv run python ./.trellis/scripts/get_context.py --mode phase --step <step>
# e.g. uv run python ./.trellis/scripts/get_context.py --mode phase --step 1.1
```
...
```bash
uv run python ./.trellis/scripts/task.py create "<task title>" --slug <name>
```
...
Skip when `uv run python ./.trellis/scripts/task.py current --source` already points to a task.
...
```bash
uv run python ./.trellis/scripts/task.py start <task-dir>
```
