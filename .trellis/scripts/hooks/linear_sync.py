...
Usage (called automatically by task.py hooks):
    uv run python .trellis/scripts/hooks/linear_sync.py create
    uv run python .trellis/scripts/hooks/linear_sync.py start
    uv run python .trellis/scripts/hooks/linear_sync.py archive

Manual usage:
    TASK_JSON_PATH=.trellis/tasks/<name>/task.json uv run python .trellis/scripts/hooks/linear_sync.py sync
