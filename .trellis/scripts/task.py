...
Usage:
    uv run python task.py create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3] [--parent <dir>] [--package <pkg>]
    uv run python task.py add-context <dir> <file> <path> [reason] # Add jsonl entry
    uv run python task.py validate <dir>              # Validate jsonl files
    uv run python task.py list-context <dir>          # List jsonl entries
    uv run python task.py start <dir>                 # Set active task
    uv run python task.py current [--source]          # Show active task
    uv run python task.py finish                      # Clear active task
    uv run python task.py set-branch <dir> <branch>   # Set git branch
    uv run python task.py set-base-branch <dir> <branch>  # Set PR target branch
    uv run python task.py set-scope <dir> <scope>     # Set scope for PR title
    uv run python task.py archive <task-dir>          # Archive completed task
    uv run python task.py list                        # List active tasks
    uv run python task.py list-archive [month]        # List archived tasks
    uv run python task.py add-subtask <parent-dir> <child-dir>     # Link child to parent
    uv run python task.py remove-subtask <parent-dir> <child-dir>  # Unlink child from parent
"""
...
Usage:
  uv run python task.py create <title>                     Create new task directory
  uv run python task.py create <title> --package <pkg>     Create task for a specific package
  uv run python task.py create <title> --parent <dir>      Create task as child of parent
  uv run python task.py add-context <dir> <jsonl> <path> [reason]  Add entry to jsonl
  uv run python task.py validate <dir>                     Validate jsonl files
  uv run python task.py list-context <dir>                 List jsonl entries
  uv run python task.py start <dir>                        Set active task
  uv run python task.py current [--source]                 Show active task
  ...
  uv run python task.py set-scope <dir> <scope>            Set scope for PR title
  uv run python task.py archive <task-dir>                 Archive completed task
  uv run python task.py add-subtask <parent> <child>       Link child task to parent
  uv run python task.py remove-subtask <parent> <child>    Unlink child from parent
  uv run python task.py list [--mine] [--status <status>]  List tasks
  uv run python task.py list-archive [YYYY-MM]             List archived tasks
...
Examples:
  uv run python task.py create "Add login feature" --slug add-login
  uv run python task.py create "Add login feature" --slug add-login --package cli
  uv run python task.py create "Child task" --slug child --parent .trellis/tasks/01-21-parent
  uv run python task.py add-context <dir> implement .trellis/spec/cli/backend/auth.md "Auth guidelines"
  uv run python task.py set-branch <dir> task/add-login
  uv run python task.py start .trellis/tasks/01-21-add-login
  uv run python task.py current --source
  uv run python task.py finish
  uv run python task.py archive add-login
  uv run python task.py add-subtask parent-task child-task  # Link existing tasks
  uv run python task.py remove-subtask parent-task child-task
  uv run python task.py list                               # List all active tasks
  uv run python task.py list --mine                        # List my tasks only
  uv run python task.py list --mine --status in_progress   # List my in-progress tasks
""")
...
def main() -> int:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "init-context":
        ...
        print(
            "  uv run python ./.trellis/scripts/get_context.py --mode phase --step 1",
            file=sys.stderr,
        )
        print(
